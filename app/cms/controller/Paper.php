...
use app\cms\model\Paper as PaperModel;

class Paper extends AdminBase
{
    /**
     * 列表管理
    ...
    {
        $list = PaperModel::order('id desc')->paginate($limit);
    ...
    public function edit($id)
    {
        if (request()->isPost()) {
            $data = input('param.');
            $result = PaperModel::update($data, ['id' => $id]);
    ...
    public function edit_state_same($id)
    {
        if (request()->isPost()) {
            $data = input('param.');
            $result = PaperModel::update(['status' => $data['status']], ['id' => $id]);
            if ($result == true) {
    ...
    public function edit_weight_same($id)
    {
        if (request()->isPost()) {
            $data = input('param.');
            $result = PaperModel::update(['weight' => $data['weight']], ['id' => $id]);
            if ($result == true) {
